defmodule PhoenixChatWeb.CoreComponents do
    ...
    <div>
      <label class="flex items-center gap-4 text-sm leading-6 text-zinc-300">
        ...
        <input
          ...
          value="true"
          checked={@checked}
          class="rounded border-zinc-600 text-zinc-100 focus:ring-0"
          {@rest}
        />
        {@label}
      </label>
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-2 block w-full rounded-md border border-zinc-600 bg-zinc-800 text-white shadow-sm focus:border-zinc-400 focus:ring-0 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
  ...
  def input(%{type: "textarea"} = assigns) do
    ...
    <div>
      ...
      <textarea
        ...
        name={@name}
        class={[
          "mt-2 block w-full rounded-lg text-zinc-100 focus:ring-0 sm:text-sm sm:leading-6",
          "min-h-[6rem] border-zinc-600 bg-zinc-800 focus:border-zinc-400",
          @errors != [] && "border-rose-400 focus:border-rose-400"
        ]}
        {@rest}
      >{Phoenix.HTML.raw(Phoenix.HTML.Form.normalize_value("textarea", @value))}</textarea>
      <.error :for={msg <- @errors}>{msg}</.error>
    </div>
    """
  end
  ...
  def input(assigns) do
    ...
    <div>
      ...
      <input
        ...
        class={[
          "mt-2 block w-full rounded-lg text-zinc-100 focus:ring-0 sm:text-sm sm:leading-6",
          "border-zinc-600 bg-zinc-800 focus:border-zinc-400",
          @errors != [] && "border-rose-400 focus:border-rose-400"
        ]}
        {@rest}
      />
  ...
  def label(assigns) do
    ~H"""
    <label for={@for} class="block text-sm font-semibold leading-6 text-zinc-100">
      {render_slot(@inner_block)}
    </label>
    """
  end
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="shadow-zinc-700/10 ring-zinc-700/10 relative hidden rounded-2xl bg-zinc-800 p-14 shadow-lg ring-1 transition"
            >
    ...
    <div class="overflow-y-auto px-4 sm:overflow-visible sm:px-0">
      <table class="w-[40rem] mt-11 sm:w-full">
        ...
        <tbody
          ...
          class="relative divide-y divide-zinc-700 border-t border-zinc-600 text-sm leading-6 text-zinc-300"
        >
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-zinc-800/50">
            <td
            ...
            >
              <div class="block py-4 pr-6">
                <span class="absolute -inset-y-px right-0 -left-4 group-hover:bg-zinc-800/50 sm:rounded-l-xl" />
                <span class={["relative", _i == 0 && "font-semibold text-zinc-100"]}>
            ...
            <td :if={@action != []} class="relative w-14 p-0">
              <div class="relative whitespace-nowrap py-4 text-right text-sm font-medium">
                <span class="absolute -inset-y-px -right-4 left-0 group-hover:bg-zinc-800/50 sm:rounded-r-xl" />
                <span
                  :for={action <- @action}
